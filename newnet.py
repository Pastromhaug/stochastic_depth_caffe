...
def residual_factory_padding1(bottom, num_filter, stride, batch_size, feature_size):
    ...
    relu = L.ReLU(concate, in_place=True)
    return relu
...
def make_net(stages, device):
    ...
    with open('/home/pas282/caffe2/caffe/examples/stochastic_depth_caffe/newnet_train.prototxt', 'w') as f:
        print(str(resnet('examples/cifar10/cifar10_train_leveldb_padding' + str(device), stages=stages, batch_size=128)), file=f)

    with open('/home/pas282/caffe2/caffe/examples/stochastic_depth_caffe/newnet_test.prototxt', 'w') as f:
...
def make_solver(niter=50000, lr = 0.1):
    ...
    s.train_net = 'examples/resnet_cifar/residual_train_per_andre.prototxt'
    s.test_net.append('examples/resnet_cifar/residual_test_per_andre.prototxt')
